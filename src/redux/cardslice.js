...
const cartSlice = createSlice({
    name: "cart",
    initialState: {
       // cartItems: JSON.parse(localStorage.getItem("posUser")).username === JSON.parse(localStorage.getItem("cart")).username
    ...
    reducers: {
        addProduct: (state, action) => {
            const findItem = state?.cartItems?.find((item) => item._id === action.payload._id); // action.payload gönderilen değer alma 
            if (findItem) {
